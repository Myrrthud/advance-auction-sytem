...
public class AuctionItem implements Serializable {
    ...
    private SellerInterface owner;
    private BuyerInterface lastBidder;
    private String lastBidderName;
    //private HashMap<String, AccountInterface> biddersMap;
    private HashMap<String, BuyerInterface> biddersMap;

    // Initialise new auction item
    public AuctionItem(long auction, String name, double minValue, Date end, SellerInterface a) {
        auctionID = auction;
        itemName = name;
    ...
    private synchronized void closeAuction() {
        ...
        try {
            if (lastBidder == null) {
                ...
                for (Entry<String, BuyerInterface> entry : biddersMap.entrySet()) {
                    BuyerInterface bidder = entry.getValue();
    ...
    /* Set method */
    public synchronized void setAuctionItemBid(BuyerInterface bidder, double bidValue) {
        try {
            // add bidder to biddersMap
            biddersMap.put(bidder.getName(), bidder);
            lastBidder = bidder;
            lastBidderName = bidder.getName();
            itemMinValue = bidValue;

            for (Entry<String, BuyerInterface> entry : biddersMap.entrySet()) {
                BuyerInterface b = entry.getValue();
                if (b == lastBidder) {
